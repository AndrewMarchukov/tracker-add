...
# Get list of active torrents
    ids="$(transmission-remote "$HOSTPORT" --authenv --list | grep -vE '^[[:space:]]+ID[[:space:]]|Seeding|Stopped|Finished|[[:space:]]100%[[:space:]]' | grep '^ ' | awk '{ gsub(/[^0-9]/,"",$1); print $1 }')"
for id in $ids ; do
    info="$(transmission-remote "$HOSTPORT" --authenv --torrent "$id" --info)"
